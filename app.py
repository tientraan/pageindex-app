...
# 2. Header
# ------------------------------
st.markdown("<h1 style='text-align: center; color: #1A73E8;'>💎 PageIndex Intelligence Analyzer</h1>", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; color: #5f6368; font-size: 1.1em;'>Hệ thống phân tích cấu trúc báo cáo dựa trên <b>Google Gemma 2 2B</b></p>", unsafe_allow_html=True)
st.divider()
...
def extract_pageindex_titles(nodes, path=""):
    ...
    for node in nodes:
        ...
        keywords = ["pageindex", "core feature", "quick hands-on", "package usage", "deployment", "configuration"]
...
if not tree_file.exists():
    st.error("❌ Không tìm thấy file 'data/data_structure.json'. Vui lòng kiểm tra lại thư mục!")
    st.stop()
...
- Nếu không thấy thông tin trong dữ liệu, hãy dựa vào kiến thức chung về PageIndex để gợi ý."""
            ...
            with st.spinner("🤖 Gemma 2 (2B) đang phân tích dữ liệu..."):
                try:
                    ...
                    st.info("Kiểm tra xem bạn đã chạy lệnh: `ollama run gemma2:2b` chưa.")
...
# Footer
st.markdown("---")
st.markdown("<center style='color: #80868b; font-size: 0.9em;'>Powered by Google Gemma 2 (2B) & Ollama • 2024</center>", unsafe_allow_html=True)